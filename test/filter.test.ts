...
describe("filterCreations", () => {
  ...
  test("search matches on title, case-insensitively", () => {
    expect(filterCreations(all, { search: "NEXUS", activeTags: [] })).toEqual([nexus]);
  });

  test("search matches on description", () => {
    expect(filterCreations(all, { search: "pan/zoom", activeTags: [] })).toEqual([graph]);
  });

  test("search matches on tags", () => {
    expect(filterCreations(all, { search: "compute", activeTags: [] })).toEqual([nexus]);
  });

  test("active tags combine with AND — an entry must have every active tag", () => {
    expect(filterCreations(all, { search: "", activeTags: ["Graph", "Scale"] })).toEqual([graph]);
    expect(filterCreations(all, { search: "", activeTags: ["Graph", "Compute"] })).toEqual([]);
  });
  ...
  test("no matches returns an empty array, not null/undefined", () => {
    expect(filterCreations(all, { search: "zzz-no-match", activeTags: [] })).toEqual([]);
  });
});
